...
async function fetchSourceRows(executionId: string): Promise<PipelineRunSource[]> {
    const sources: TenderSource[] = ['sam', 'ted', 'calusource', 'uofa', 'nyscr'];
...
function evaluateRules(runs: PipelineRunSummary[], latestSources: PipelineRunSource[]): Alert[] {
    ...
    if (runs.length >= 2) {
        for (const src of ['sam', 'ted', 'calusource', 'uofa', 'nyscr'] as TenderSource[]) {
    ...
    if (latest.status !== 'FAILED') {
        const expectedSources = latest.sourcesAttempted ?? ['sam', 'ted', 'calusource', 'uofa', 'nyscr'];
