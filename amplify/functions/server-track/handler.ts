            ...
            if (aiResult) {
                ...
                await getDynamoClient().send(new UpdateCommand({
                    ...
                    ExpressionAttributeValues: {
                        ...
                        ':isTgt': finalIsTargetCustomer || undefined,
                        ':conf': aiResult.confidence,
                        ':orgType': aiResult.organizationType,
                        ':leadTier': finalLeadTier || undefined,
...
async function writeAIEnrichment(
    ...
    const doUpdate = () => getDynamoClient().send(new UpdateCommand({
        ...
        ExpressionAttributeValues: {
            ':aiOrgType': props.aiOrganizationType || undefined,
            ':aiConf': props.aiConfidence ?? undefined,
            ':aiReason': props.aiReason || undefined,
            ':isTgt': props.isTargetCustomer === true || undefined,
            ':conf': props.confidence ?? undefined,
            ':orgType': props.organizationType || undefined,
            ':leadTier': props.leadTier || undefined,
            ':updatedAt': now,
        },
        ConditionExpression: 'attribute_exists(id)',
    }));
