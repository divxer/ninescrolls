...
- [ ] **Step 2: §5-dominance grep + Gate-C (manual).**
  ...
  - **Gate-P3 — Principle Dominance (NEW, template-reusable):** delete the principle SENTENCE ("…a stopping problem, not a removal problem") and re-read §1–§5. Do the sections still auto-converge into the same single story (every step judged by where it lands on the stopping window)? They MUST — the principle should be the *organizing structure* the sections already embody, not a slogan bolted on top. If removing the sentence makes the page read as unrelated steps, the principle is decorative: rewrite the sections so the stopping-problem framing is load-bearing in each, not just stated in §5.
