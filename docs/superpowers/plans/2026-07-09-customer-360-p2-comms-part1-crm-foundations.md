...
await docClient.send(new TransactWriteCommand({ TransactItems: [
  ...
  { Update: { TableName: TABLE_NAME(), Key: reconKey(fromOrgId, toOrgId),   // UPSERT visibility marker — no Put collision can cancel the transaction; upsert implicitly supersedes a prior pair's marker
      ...
      UpdateExpression: 'SET #st = :probe, GSI1PK = :gpk, GSI1SK = :gsk, entityType = :et, fromOrgId = :from, toOrgId = :to2, mergedAt = :now, residualsDetected = :null, residualSamples = :empty, lagHorizonAt = :horizon, updatedAt = :now, createdAt = if_not_exists(createdAt, :now), version = if_not_exists(version, :zero) + :one',
...
```
(**Scope note — ratified 2026-07-22:** the marker is VISIBILITY-ONLY; states `pending_probe → needs_review → acknowledged`, never `complete`. See Task 12 for the probe. No residual draining, no aggregate rebuilds, no historical-merge backfill — those belong to the future "Organization Merge Integrity" arc; the marker's evidence fields make that arc possible without schema migration.)
     Cancellation outcomes (positional): index 0 (target not active) ⇒ the R5 non-active-target rejection; index 1 (source not active) ⇒ read the source — `mergedInto === toOrgId && mergePhase === 'complete'` ⇒ early-return success (pinned production behavior); `mergedInto === toOrgId && mergePhase === 'archived'` ⇒ **RESUME** the incomplete phases; `mergedInto !== toOrgId` ⇒ explicit error `already merged into <mergedInto>` (a competing different-target merge won). Tests: competing same-pair merges (one archives, one resumes), competing different-target merges (one wins, the other errors with the winner's target), target archived concurrently (index-0 rejection).
...
**Merge-residual VISIBILITY probe (scope cut ratified 2026-07-22 — replaces the reconciliation state machine; no draining, no convergence claims, no `complete`):** each cycle queries `GSI1PK='MERGE_RECON#pending_probe'` markers past `lagHorizonAt` (bounded). Per marker, ONE **bounded first-page probe** of the from-org partition (records + Contacts + timeline events sub-queries, `Limit ~20` each, NO pagination — this measures presence, not totals):
  ...
  - Probe read failure ⇒ transient: state unchanged, retried next cycle. `needs_review` markers are NOT re-probed; the admin acknowledges them in CRM Health (`needs_review → acknowledged` — a Task 13 mutation-side follow-up is NOT in this plan; v1 surfaces the count/samples read-only).
