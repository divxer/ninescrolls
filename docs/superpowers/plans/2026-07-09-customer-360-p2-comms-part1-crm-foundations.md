...
Marker-upsert tests (R9): (g) `acknowledged → pending_probe` supersession — re-merging an acknowledged pair resets state AND removes `probedAt`/`acknowledgedBy`/`acknowledgedAt` (assert the attributes are gone from the store, not merely overwritten); (h) transaction retry after a crash mid-flow is idempotent — the second upsert bumps `version` but yields the same state/fields.
  ...
  3. **Resume semantics (R6 blocker 3).** The already-archived-into-same-target early return fires ONLY on `mergePhase === 'complete'`; on `'archived'` the retry resumes the INTERACTIVE phases — re-pointing best-effort pass, domains, aggregation dirty-marks, exactly production's existing behavior (all conditional ⇒ idempotent) — and sets `mergePhase: 'complete', mergeCompletedAt` LAST. **`mergePhase:'complete'` means the interactive flow finished — it is NOT a convergence claim**; residual visibility is the Task 12 probe's job, and full convergence is explicitly out of scope (ratified cut).
...
**Merge-residual VISIBILITY probe (scope cut ratified 2026-07-22 — replaces the reconciliation state machine; no draining, no convergence claims, no `complete`). Exact mechanics (R9):**
  ...
  - **Per-marker probe:** exactly THREE first-page queries on `GSI2: GSI2PK = 'ORG#<fromOrgId>'` with `begins_with(GSI2SK, <prefix>)` for the record/contact/event key prefixes (locate the exact prefixes in `keys.ts`), each `Limit: 20`, `ScanIndexForward: true`, **NO `ExclusiveStartKey` — ever** (presence measurement, not totals; assert in tests that no follow-up page is requested).
  - **Row treatment:** `LINK_AUDIT`-prefixed rows are skipped (known-immutable, never counted). ANY other row — including unknown entity types — counts toward `residualsDetected` and the sample (visibility, not classification; unknowns are exactly what the admin should see). Sample cap: **10 PKs total across the three queries**, first-come.
  ...
  - `crmHealth` gains `mergeNeedsReviewCount` (+ the samples of the newest such marker): extend `lib/repair/crmHealth.ts` and its test IN THIS TASK; check whether the existing `/admin/crm-health` page renders summary fields generically — if not, add the one display row in Task 13's frontend step.
...
// Scenario 16 — STUCK RECOVERY end-to-end: blocked marker heals automatically once the chain is fixed.
