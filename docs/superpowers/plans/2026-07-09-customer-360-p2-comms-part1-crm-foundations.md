...
```
Append to `handler.test.ts` (plan-review fix: cover **ALL** guarded resolvers × three identity cases; reuse its mocks; reject BEFORE the lib fn is called). The GUARDED list starts with six entries here; **Task 13 Step 2b adds `acknowledgeMergeRecon` as the seventh** — extend this same array there:
```ts
...
**Write-time active fence — COMPLETE coverage (R5 blocker 1, extended R6 blockers 1–2):** **every org-referencing generational write in the replay** is issued as a `TransactWriteItems` whose element 0 is the org fence (import `orgActiveCheck` from Task 10):
```ts
// source backfill:      TransactWriteItems([orgActiveCheck(eff), { Update: <backfillByPk update, unchanged expressions> }])
// contact:              upsertContact({ ..., orgId: eff, linkGeneration: generation, activeOrgFence: true })   // Task 7's fence param
// audit put:            TransactWriteItems([orgActiveCheck(eff), { Put: <audit item, attribute_not_exists(PK)> }])
// redirected event move: TransactWriteItems([orgActiveCheck(eff), { Put: <moved item, orgId=:requested AND linkGeneration=:gen> }])
// rollup-dirty writes:  TransactWriteItems([orgActiveCheck(<that org>), { Update: <dirty mark> }])   // fence the ACTIVE-target org's write; the archived-side dirty mark is unfenced (its org is legitimately archived)
```
Cancellations are classified **positionally** (`classifyLinkCancellation` from Task 10 — index 0 = fence, index 1 = the write): fence failure ⇒ **re-run `resolveEffectiveTarget` once** (the org was archived between resolve and write — its successor is now visible) and retry the whole remaining effect sequence against the new effective org (fresh fences); a SECOND fence failure ⇒ `transient` (a merge storm is in progress — retry next drain). A write-condition failure (index 1) maps to the existing outcomes (`superseded`/`already_set`/`conflict`/skip) exactly as before. Tests: audit + redirect-move + rollup writes each carry the fence at index 0 (extend the Step-1 fence test to assert ALL transact calls in a redirect replay have `ConditionCheck` first); an audit-write fence failure re-resolves and re-runs the audit at the successor.
...
- [ ] **Step 4: Merge side — spec R10 FINAL (R5-hardened): four bounded changes to the located `mergeOrganization` flow.**
  ...
  2. **ONE atomic archive transaction (R7 blockers 1+3 — the linearization point).** Replace the separate archive/marker writes with a single `TransactWriteItems`:
```ts
await docClient.send(new TransactWriteCommand({ TransactItems: [
  { ConditionCheck: { TableName: TABLE_NAME(), Key: orgKey(toOrgId),
      ConditionExpression: '#s = :active', ExpressionAttributeNames: { '#s': 'status' }, ExpressionAttributeValues: { ':active': 'active' } } },
  { Update: { TableName: TABLE_NAME(), Key: orgKey(fromOrgId),
      // R8 blocker 2: this is the PRODUCTION archive UpdateExpression — locate it verbatim and EXTEND it.
      // It must RETAIN everything production sets today (mergedAt, updatedAt, and its REMOVE of the
      // active/activity index keys) and ADD mergedInto + mergePhase. Example shape (adjust to the real one):
      UpdateExpression: 'SET #s = :archived, mergedAt = :now, updatedAt = :now, mergedInto = :to, mergePhase = :ph REMOVE <the production active-index keys, verbatim>',
      ConditionExpression: '#s = :active',                       // EXACT source state — competing merges linearize HERE
      ExpressionAttributeNames: { '#s': 'status' },
      ExpressionAttributeValues: { ':archived': 'archived', ':now': nowIso, ':to': toOrgId, ':ph': 'archived', ':active': 'active' } } },
  { Update: { TableName: TABLE_NAME(), Key: reconKey(fromOrgId, toOrgId),   // UPSERT visibility marker — no Put collision can cancel the transaction; upsert implicitly supersedes a prior pair's marker
      // R8 blocker 1: the marker MUST be discoverable — full index metadata, entity type, timestamps.
      // R9: a superseding upsert (same pair re-merged after acknowledge) must SCRUB stale probe/ack metadata
      UpdateExpression: 'SET #st = :probe, GSI1PK = :gpk, GSI1SK = :gsk, entityType = :et, fromOrgId = :from, toOrgId = :to2, mergedAt = :now, residualsDetected = :null, residualSamples = :empty, lagHorizonAt = :horizon, updatedAt = :now, createdAt = if_not_exists(createdAt, :now), version = if_not_exists(version, :zero) + :one REMOVE probedAt, acknowledgedBy, acknowledgedAt',
      ExpressionAttributeNames: { '#st': 'state' },
      ExpressionAttributeValues: { ':probe': 'pending_probe', ':gpk': 'MERGE_RECON#pending_probe', ':gsk': nowIso, ':et': 'MERGE_RECON', ':from': fromOrgId, ':to2': toOrgId, ':now': nowIso, ':null': null, ':empty': [], ':horizon': lagHorizonIso, ':zero': 0, ':one': 1 } } },
]}));
```
(**Scope note — ratified 2026-07-22:** the marker is VISIBILITY-ONLY; states `pending_probe → needs_review → acknowledged`, never `complete`. See Task 12 for the probe, Task 13 for the acknowledge mutation. No residual draining, no aggregate rebuilds, no historical-merge backfill — those belong to the future "Organization Merge Integrity" arc; the marker's evidence fields make that arc possible without schema migration.)
Marker-upsert tests (R9, corrected R10-final-gate — a post-archive retry can never re-run the upsert, so test what actually happens): (g) **first archive of a pair whose marker is `acknowledged`** (the source was un-archived/restored out-of-band or this is a re-created org) resets the marker to `pending_probe` AND removes `probedAt`/`acknowledgedBy`/`acknowledgedAt` (assert the attributes are GONE from the store item, not merely overwritten); (h) **crash after the archive transaction ⇒ retry takes the index-1 RESUME path**: the marker item and its `version` are asserted UNCHANGED while the remaining phases run to completion — the upsert is provably not re-executed.
     Cancellation outcomes (positional): **index 0 (target not active) ⇒ READ THE SOURCE FIRST (R9 critical — chained merge during resume):** if the source is archived with `mergedInto === toOrgId` and `mergePhase !== 'complete'`, this is a RESUME whose target was itself merged away (`A→B` crashed, then `B→C` completed) — resolve `resolveEffectiveTarget(toOrgId)` and **finish the source's remaining interactive phases against the ACTIVE successor** (org-fenced writes; `A.mergedInto` stays `B` — historical truth, the successor chain resolves it; flip `mergePhase:'complete'` on A last); successor unavailable ⇒ explicit error, A stays incomplete and visible via its recon marker. Only when the source is ACTIVE is index-0 the genuine R5 non-active-target rejection. Index 1 (source not active) ⇒ read the source — `mergedInto === toOrgId && mergePhase === 'complete'` ⇒ early-return success (pinned production behavior); `mergedInto === toOrgId && mergePhase === 'archived'` ⇒ **RESUME** the incomplete phases; `mergedInto !== toOrgId` ⇒ explicit error `already merged into <mergedInto>` (a competing different-target merge won). Tests: competing same-pair merges (one archives, one resumes), competing different-target merges (one wins, the other errors with the winner's target), target archived concurrently with an ACTIVE source (genuine rejection), and the EXACT chained sequence **`A→B` crash-after-archive → `B→C` completes → retry `A→B` ⇒ A's phases finish against C, `A.mergedInto` still `B`, `A.mergePhase:'complete'`**.
  3. **Resume semantics — ONE shared path (R6 blocker 3, exact contract R10-final-gate):** every resume (plain or chained) runs
```ts
async function runRemainingMergePhases(args: { sourceOrgId: string; requestedTargetOrgId: string; effectiveTargetOrgId: string }): Promise<void>
```
     — target usage is explicit per operation: **record/Contact/event re-pointing writes → `effectiveTargetOrgId`**, each as `TransactWriteItems([orgActiveCheck(effectiveTargetOrgId), <the conditional re-point>])`; **domain re-pointing → `effectiveTargetOrgId`**, fenced identically; **aggregation dirty-marks → `effectiveTargetOrgId` (fenced) + `sourceOrgId` (unfenced — legitimately archived)**; **logs/audit details → record BOTH `requestedTargetOrgId` and `effectiveTargetOrgId`** when they differ. Cancellations are classified positionally: **index 0 (effective-target fence) ⇒ the effective target was itself merged mid-resume (C→D) — re-run `resolveEffectiveTarget(requestedTargetOrgId)` ONCE and re-enter `runRemainingMergePhases` with the new effective org; a second fence failure ⇒ abort with a transient error, `mergePhase` stays `'archived'` (retryable, visible via the recon marker)**; index 1 (the write's own condition) ⇒ idempotency — the item was already re-pointed — skip and continue (NEVER confused with the fence). `mergePhase: 'complete', mergeCompletedAt` is set on `sourceOrgId` LAST, only after the phase run finishes. The plain resume is simply `effectiveTargetOrgId === requestedTargetOrgId`. **`mergePhase:'complete'` means the interactive flow finished — it is NOT a convergence claim**; residual visibility is the Task 12 probe's job, and full convergence is explicitly out of scope (ratified cut). Tests: target-usage matrix (each operation asserted against the right org), C→D mid-resume re-resolution, second-fence-failure abort leaves `mergePhase:'archived'`, idempotent re-run skips already-moved items.
  ...
  **Why this is safe (document in the merge test file header):** already-applied replays supersede on their own stamp; not-yet-applied replays are fenced at write time and redirected (or blocked, never falsely completed); archive-first makes the fence airtight and the re-drain catches the fenced-in-flight window. Clock skew never decides a merge outcome because no stamp comparison is involved.
  Merge-side tests: (a) archive write persists `mergedInto` + `mergePhase:'archived'` and the `MERGE_RECON` marker in the same step; (b) archive happens BEFORE the first re-point write (assert call order); (c) crash-after-archive ⇒ retry resumes phases and flips `mergePhase:'complete'` LAST; (d) merging INTO an archived target throws; (e) the stamp-untouched regression guard; (f) already-archived-into-same-target with `mergePhase:'complete'` ⇒ early-return (pinned), with `mergePhase:'archived'` ⇒ resumes (NOT early-return).
...
export async function linkStructuredUnit(args: { representativeEventId: string; targetOrgId: string; operator: string }): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    const cls = classifyLinkCancellation(err, 1);         // Task-10 helper: POSITIONAL CancellationReasons mapping (org-fence index 0, move index given)
...
```
Include the three small helpers in the same file: `orgActiveCheck(orgId)` (the R6 fence element — `{ConditionCheck: {Key: orgKey(orgId), ConditionExpression: '#s = :active', …}}`; exported, Task 8's replay imports it); `buildMoveTransactPut` (the movedItem exactly as `manualMoveTimelineEvent` constructs it, wrapped as a `Put` with the R7 eligibility condition — extract the shared item-construction into an exported `buildMovedItem(event, targetOrgId, email, nowIso, linkGeneration)` in `manualMoveTimelineEvent.ts` so the two paths cannot drift); and `classifyLinkCancellation(err, moveIndex)` → `'org_fence' | 'move_condition' | 'other'` (POSITIONAL: `err.CancellationReasons[0].Code === 'ConditionalCheckFailed'` ⇒ `org_fence`; `[moveIndex]` ⇒ `move_condition`; non-`TransactionCanceledException`, a MISSING/malformed `CancellationReasons` array, or anything else ⇒ `other` — R6/R7: never `.some(…)` across positions, never guess when the array is absent; `other` propagates as an error/transient). **`manualMoveTimelineEvent` itself becomes a 2-item fenced transaction** `[orgActiveCheck(targetOrgId), {Put: movedItem}]` with the same positional classification (org-fence ⇒ throw the merged-org error; move-condition ⇒ `{skipped:true}` as today).
...
**Recovery pass (R5 blocker 3 — `markStuck` removes the marker from the pending query, so a fixed org chain must be actively re-checked):** each drain cycle ALSO runs `queryStuckByReason('target_unavailable', 25)` — a **keyed-partition query** (`GSI1PK = 'CRM_REPAIR#stuck#target_unavailable'`, Task 9), NOT a filter over a shared stuck partition (R6: DynamoDB applies `Limit` before `FilterExpression`, so a large 'other' backlog would starve recovery). For each hit, call `resolveEffectiveTarget(marker.targetOrgId)` — if it now resolves `active`, `republishStuckFenced` (stuck→pending; `{lost:true}` ⇒ skip) so the NEXT drain repairs it normally; if still unavailable, leave it. Summary gains `recovered`.
**Merge-residual VISIBILITY probe (scope cut ratified 2026-07-22 — replaces the reconciliation state machine; no draining, no convergence claims, no `complete`). Exact mechanics (R9):**
  - **Marker discovery:** `Query GSI1: GSI1PK = 'MERGE_RECON#pending_probe' AND GSI1SK <= :horizonCutoff` where `:horizonCutoff = now - 15min` (GSI1SK stores `mergedAt`, so the horizon condition is a KEY condition — markers before the lag horizon are never even read), `ScanIndexForward: true` (oldest first), `Limit: 10` markers per cycle.
  - **Per-marker probe: FIVE explicit first-page queries** (R10-final-gate — three prefixes cannot cover the schema) on `GSI2: GSI2PK = 'ORG#<fromOrgId>'` with `begins_with(GSI2SK, <prefix>)` for each of **`RFQ#`, `ORDER#`, `LEAD#` (source records), `CONTACT#`, `TLEVENT#`** (verify the exact prefix strings against `keys.ts` before writing — if the schema carries additional org-partition prefixes beyond these and `AUDIT#`, add a bounded query per discovered prefix and note it in the task report), each `Limit: 20`, `ScanIndexForward: true`, **NO `ExclusiveStartKey` — ever** (presence measurement, not totals).
  - **Row treatment:** `AUDIT#`/`LINK_AUDIT`-prefixed rows are skipped (known-immutable, never counted). ANY other row — including unknown entity types — counts toward `residualsDetected` and the sample (visibility, not classification; unknowns are exactly what the admin should see). Sample cap: **10 PKs total across the five queries**, first-come.
  - **Probe command-shape tests (expanded, R10-final-gate):** assert (i) exactly FIVE queries per probed marker, one per prefix, in a fixed order; (ii) every query carries `Limit: 20`, `ScanIndexForward: true`, and NO `ExclusiveStartKey` (and no follow-up page is ever requested even when a page is full); (iii) audit rows present ⇒ excluded from both `residualsDetected` and samples; (iv) an unknown-entityType row ⇒ counted + sampled; (v) 30 residuals across prefixes ⇒ `residualSamples` capped at exactly 10; (vi) the state-flip Update's `ConditionExpression` is `'#st = :probe AND version = :v'` and a CCFE ⇒ marker untouched by THIS actor, no retry within the cycle.
  - **State flip:** one version-fenced Update — `SET #st=:review, GSI1PK=:reviewPk, residualsDetected=:d, residualSamples=:s, probedAt=:now, updatedAt=:now, version=version+:one` with `ConditionExpression: '#st = :probe AND version = :v'`; CCFE ⇒ another actor advanced it ⇒ skip silently.
  - **Failure:** any probe read throwing ⇒ marker untouched (state/version unchanged), retried next cycle.
  Detail:
  - `LINK_AUDIT` rows are **known-immutable, non-touching, non-blocking** — never counted as residuals (they legitimately share the org index).
  - Set `residualsDetected: true|false` **from what was actually measured** and `residualSamples` (≤10 PKs, diagnostics for the admin and for the future merge-integrity arc); version-fenced flip `pending_probe → needs_review` (`GSI1PK='MERGE_RECON#needs_review'`).
  - Probe read failure ⇒ transient: state unchanged, retried next cycle. `needs_review` markers are NOT re-probed; the admin acknowledges them via **Task 13's `acknowledgeMergeRecon` mutation**.
  - `crmHealth` gains `mergeNeedsReviewCount` AND a **bounded `mergeReviewMarkers` list (≤20, oldest first)** with at least `{ fromOrgId, toOrgId, version, residualsDetected, residualSamples, probedAt }` per marker — the acknowledgement UI (Task 13 Step 2b) renders one row + button per entry: extend `lib/repair/crmHealth.ts` and its test IN THIS TASK (one bounded GSI1 Query on `MERGE_RECON#needs_review`, mapped to the item shape).
  - **Explicitly NOT here (accepted v1 risk, documented in the spec):** no automatic residual draining; no bulk TimelineEvent/Contact/domain migration beyond production merge behavior; no aggregate rebuilds; no historical-merge backfill; the 3C sweep is NOT claimed to heal archived-org residuals. Already-completed links' events may remain under a subsequently archived org — manual admin correction.
Also (R7): the STUCK-recovery query gains a **persisted rotation cursor** stored on the existing 3C drain state item and updated under its lease/fence (`ExclusiveStartKey`, wrapping to null at partition end) so 25 still-unavailable markers at the head can't starve the tail.
Tests: (e) marker past horizon ⇒ probed ONCE (first page only — assert no pagination follow-up), `residualsDetected`+samples set from the seeded store, state → `needs_review`; (f) `LINK_AUDIT` rows in the partition ⇒ NOT counted, `residualsDetected:false` when they're all that remains; (g) marker before `lagHorizonAt` ⇒ untouched; (h) probe read failure ⇒ state unchanged, retried next cycle; (i) `needs_review` marker ⇒ never re-probed; (j) stuck-rotation: 30 head markers still-unavailable ⇒ the tail is reached on the next cycle via the persisted cursor, cursor wraps at partition end.
...
// everything to the successor (marker-driven convergence).
it('mid-unit archive: fenced later moves stop; the marker drives full convergence at the successor', async () => {
  ...
  const gate = store.gateOn((cmd, input, n) => n === 2 && cmd.constructor.name === 'TransactWriteCommand');  // parks the SECOND move
  const pLink = linkStructuredUnit({ representativeEventId: 'tev-1', targetOrgId: 'acme.com', operator: 'op' });
  await store.idle();
  archiveOrgInStore(store, 'acme.com', { mergedInto: 'b.com' });
  gate.release();
  const out = await pLink;
  expect(out.postCommitStatus).toBe('post_commit_failed');          // surfaced, marker retained
...
```bash
OUT=$(npx tsc --noEmit 2>&1); ST=$?; NEW=$(printf '%s\n' "$OUT" | grep -v "main.tsx" | grep "error TS"); if [ -n "$NEW" ]; then printf 'NEW TS ERRORS:\n%s\n' "$NEW"; false; elif [ "$ST" -ne 0 ] && ! printf '%s' "$OUT" | grep -q "main.tsx"; then printf 'tsc failed abnormally (exit %s):\n%s\n' "$ST" "$OUT"; false; else echo "PASS: only pre-existing main.tsx diagnostics (tsc exit $ST)"; fi
```
This command **exits nonzero by itself** in BOTH failure modes (R5+R6): (a) any `error TS` line outside `main.tsx`; (b) a nonzero tsc exit whose output does NOT contain the known `main.tsx` diagnostic — i.e. an abnormal tsc failure (crash, OOM, config error) can never masquerade as a pass just because it printed no matching error lines. Exits 0 with an explicit PASS (echoing the real tsc status) only when the sole diagnostics are the pre-existing `main.tsx` ones.
