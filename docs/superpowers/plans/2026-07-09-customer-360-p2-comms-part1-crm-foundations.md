...
await docClient.send(new TransactWriteCommand({ TransactItems: [
  ...
  { Update: { TableName: TABLE_NAME(), Key: orgKey(fromOrgId),
      UpdateExpression: 'SET #s = :archived, mergedInto = :to, mergePhase = :ph',
      ConditionExpression: '#s = :active',                       // EXACT source state — competing merges linearize HERE
      ExpressionAttributeNames: { '#s': 'status' },
      ExpressionAttributeValues: { ':archived': 'archived', ':to': toOrgId, ':ph': 'archived', ':active': 'active' } } },
  { Update: { TableName: TABLE_NAME(), Key: reconKey(fromOrgId, toOrgId),   // UPSERT — a completed prior marker is re-opened, no Put collision can cancel the transaction
      UpdateExpression: 'SET #st = :pending, emptyPasses = :zero, unknownEntities = :zero, lagHorizonAt = :horizon, passCursor = :null, version = if_not_exists(version, :zero) + :one',
      ExpressionAttributeNames: { '#st': 'status' },
      ExpressionAttributeValues: { ':pending': 'pending', ':zero': 0, ':one': 1, ':horizon': lagHorizonIso, ':null': null } } },
]}));
```
  ...
  3. **Resume semantics (R6 blocker 3).** The already-archived-into-same-target early return fires ONLY on `mergePhase === 'complete'`; on `'archived'` the retry resumes re-pointing/domains/aggregation (all conditional ⇒ idempotent) and sets `mergePhase: 'complete', mergeCompletedAt` LAST. The interactive re-pointing pass stays best-effort — the DURABLE convergence guarantee is Task 12's reconciliation pass; no empty immediate pass is ever treated as proof.
...
**Merge-reconciliation pass (R6 blocker 4, R7 blocker 2 — Task 8's `MERGE_RECON#` markers are drained HERE, same cron; `complete` needs EVIDENCE, not an eventually-consistent hope):** each cycle queries pending `MERGE_RECON` markers past `lagHorizonAt` (keyed partition `GSI1PK='MERGE_RECON#pending'`, bounded). Marker pass-state (all updates version-fenced): `{ passCursor, emptyPasses, unknownEntities }`. Per marker, per cycle:
  1. Enumerate the from-org partition **from the beginning** (the pass MUTATES the GSI it pages, so a single traversal can skip rows — every cycle restarts at the top), paginating with `LastEvaluatedKey` past the 1 MB limit; persist `passCursor` after each page (a crashed pass resumes mid-enumeration, then the NEXT full pass still restarts from the beginning).
  2. **Exact conditional rewrites** (specify verbatim in the implementation, locating the real GSI attribute names first — `git grep -n "GSI2PK\|GSI3PK" amplify/functions/crm-api/lib/`): source records `SET matchedOrgId = :to` + every org-derived GSI attribute (e.g. `GSI2PK = ORG#<to>`), `ConditionExpression 'matchedOrgId = :from'`, **stamps untouched** (no `matchedOrgLinkGeneration` in the UpdateExpression); Contacts `SET orgId = :to, GSI2PK = …` conditioned `orgId = :from`, `lastLinkGeneration`/`linkLocked` untouched; TimelineEvents `SET orgId = :to, GSI2PK = …` conditioned `orgId = :from` (`linkGeneration` preserved). CCFE per item ⇒ raced (another actor moved it) ⇒ re-read, count, continue.
  3. **Unknown entity types BLOCK completion** (R7): any enumerated item whose `entityType` is not in the known re-point set increments `unknownEntities`, is logged (`crm.merge.recon.unknown` with PK/entityType), and is NEVER silently skipped — the marker cannot complete while `unknownEntities > 0` on the latest pass; it surfaces in CRM Health for a human.
  4. **Domains:** paginated + conditional re-point of the from-org's domain mappings in the same pass.
  5. A pass that touched ANY item (or found unknowns) resets `emptyPasses = 0`; a fully-empty pass increments it. After any touching pass, **recompute BOTH orgs' aggregates via the existing 3C rollup-recompute path** — recomputation derives from enumerated source data, so repeated passes are idempotent (nothing is ever incremented during reconciliation — add a test proving two consecutive touching passes yield identical aggregates, no double-count).
  6. **`complete` requires `emptyPasses >= 2`** — two consecutive fully-empty verification passes in SEPARATE cron cycles (≥ the cron interval apart, beyond GSI lag), version-fenced flip. Until then `crmHealth` reports the marker as an INCOMPLETE merge (Task 13 summary gains the count).
Also (R7): the STUCK-recovery query gains a **persisted rotation cursor** on the drain state item (`ExclusiveStartKey`, wrapping at partition end) so 25 still-unavailable markers at the head can't starve the tail.
Tests: (e) residual visible only after the lag horizon (harness GSI-lag simulation) ⇒ re-pointed, aggregates recomputed AFTER, and completion requires two further empty cycles (assert `status` stays pending through the touching cycle and the FIRST empty cycle); (f) pagination past a simulated 1 MB boundary covers every item; (g) competing merge (a→b while b→c) re-points a's residuals through `resolveEffectiveTarget` to the current successor; (h) marker before `lagHorizonAt` ⇒ untouched; (i) unknown entityType ⇒ `unknownEntities` counted, completion blocked, health surfaced; (j) idempotent-aggregates double-pass test; (k) stuck-rotation: 30 head markers still-unavailable ⇒ the 26th–41st are reached on the next cycle via the persisted cursor.
