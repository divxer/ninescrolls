...
- [ ] **Step 1: Dispatch subagent.** Insert §4 between §3 and §6 (file order: …§3, §4, §5 placeholder comment, §6…— instruct the agent to insert at the marked position; simplest: Task 2's agent leaves `<!-- §4 §5 here -->` after §3, this task replaces it).
  **§4 `<h2>4. Via-First vs Via-Middle vs Via-Last</h2>` (800–900w):**
  ...
  - Primary asset figure block (prefix `tsv-integration-selection-matrix`) + how-to-read sentence.
  - **HEDGE HARD RULE (whole §4): no absolute adoption claims.** Never "HBM uses via-middle" — always "commonly/typically/often" (generations and vendors differ). Greppable spot-check: `grep -icE "(HBM|CIS|logic) (uses|requires|is built with)" f` should be 0; every adoption claim carries a hedge word.
...
- [ ] **Step 1: Dispatch subagent.** Append after §4:
  ...
  - **§8 `<h2>8. Key Takeaways</h2>` (~150w):** entity recap + the timing principle + density-is-KOZ-bound + design-space anchors. **Final takeaway = the Future Hub Hook (verbatim-close):** "A TSV is only one part of the vertical interconnect stack. The next design decisions — TSV reveal, copper filling, and bonding strategy — each deserve their own treatment." (Pre-seeds the future TSV sub-pages: Reveal, Cu Filling, Reliability, Interposer.)
...
- [ ] **Step 2: FULL AUDIT** — every row of the rule→grep table, incl. per-block word counts (R4 ≤150, FAQ 70–90), R5 total 3,100–3,500. Over → trim §1/§7/FAQ, never §4. Under → deepen §4/§5.
  **Plus the Density Audit:** `grep -ioc "TSV density\|via density" f` must be **≥** `grep -ioc "TSV etch" f` — the page must leave readers thinking "TSV = design-space problem", not "TSV = etching problem". If the ratio fails, the framing has drifted; rebalance prose (usually §2/§5/§7 density language), don't just delete etch mentions.
