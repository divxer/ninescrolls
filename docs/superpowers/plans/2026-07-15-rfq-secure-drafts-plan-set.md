...
> into five sequenced plans rather than one. Each plan produces working, testable software
...
| **P2 — Draft data model & storage** | 1 | Single-table draft item (`status=draft`, `draftTokenHash`, `draftVersion`, `lastActivityAt`/`expiresAt`/`TTL`, `GSI1PK=RFQ_STATUS#draft`); storage helpers: conditional create (idempotent on nonce), optimistic-concurrency update, whitelisted read. Non-disclosing `404 {"error":"Draft unavailable"}`. | P1 |
