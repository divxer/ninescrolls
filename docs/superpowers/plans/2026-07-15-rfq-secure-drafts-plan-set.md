...
- **P4b** — the live `submit-rfq` refactor. [`…p4b-submit-refactor.md`](2026-07-15-rfq-secure-drafts-p4b-submit-refactor.md) is a **⛔ BLOCKED historical/restructure record only** — its original draft was found not implementable in review and has been neutered. The actual work is four phases that are **not yet written and not yet reviewed**:
  - **P4b-1** — receipt-as-transaction-item + atomic submit transaction + dependency-ordered outbox model (pure, dark). First draft [`…p4b1-…`](2026-07-15-rfq-secure-drafts-p4b1-transaction-outbox-model.md) **⛔ failed review (architectural); superseded — needs redesign** (event-progressable outbox, complete upgrade projection, typed pending-item builder)
