...
> **Scope change during PR #223 review (Option 1):** P1 `resolveLinks` resolves against **existing orgs only**, using the canonical eTLD+1 identity owned by `organization-api` (`classifyEmailDomain` + `GSI2 ORG_DOMAIN#<domain>` / `ORG_DOMAIN_LOOKUP`). CRM creates **no** orgs in P1 — a corporate domain with no existing org resolves to `unresolved` (→ Needs-Linking). Removed from P1: `createReviewOrgFromDomain`, `generateOrgId`, the bespoke `ORGDOMAIN#`/`ORGNAME#` indexes, `organization_name_match`, and `email_domain_new` auto-create. CRM org rollups write the authoritative `ORG#<eTLD+1>/META` row. **Auto-create (`email_domain_new` → review org) and `organization_name_match` move to P2**, built on a shared/canonical `organization-api` upsert path (NOT a CRM re-implementation).

**Plan 2 — Channel wiring + backfill + sweep:**
- **Auto-create review orgs** (`email_domain_new`) + **`organization_name_match`** — implement here via the shared/canonical `organization-api` upsert (eTLD+1 ids, `ORG_DOMAIN_LOOKUP` aliases), so CRM never mints a duplicate of an existing company. Extract a shared org-upsert lib or have CRM invoke `organization-api`.
