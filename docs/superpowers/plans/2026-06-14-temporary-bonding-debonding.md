...
- [ ] **Step 1: Dispatch subagent.** Append:
  ...
  - **§5 `<h2 id="debond-selection">5. Debonding Method Selection</h2>` (800–900w) — THE CLIMAX:** four methods, each its own `<h3>`, argued (NOT just described) across thermal budget, throughput, device/mechanical sensitivity, residue & cleanup, cost:
    Each method MUST be led by its OWN unique FIRST constraint (the Method Inversion Test below depends on this — the methods are NOT interchangeable categories; each one exists to answer a different dominant question first):
    - `<h3>Laser Debonding</h3>` — **first constraint: mechanical/device sensitivity.** Laser exists to release a fragile device at room temperature with near-zero applied force — a release layer absorbs laser energy through a transparent carrier. Chosen first when the thinned device cannot take heat OR force; the costs (transparent carrier + laser tool, residue/cleanup) are what you pay for that gentleness.
    - `<h3>Thermal-Slide Debonding</h3>` — **first constraint: thermal budget.** Heat softens the adhesive and the carrier slides off — simple and high-throughput, but it SPENDS thermal budget and applies shear. Chosen first when the device still tolerates heat; ruled out the moment the remaining thermal budget is gone, regardless of every other advantage.
    - `<h3>Mechanical (Peel) Debonding</h3>` — **first constraint: load/force tolerance.** Room-temperature peel/lift — fastest and most tool-light, but the most mechanically aggressive. Chosen first when the stack can take the force; the question is purely how much load the thinned device survives.
    - `<h3>Chemical (Solvent) Debonding</h3>` — **first constraint: time & material compatibility.** Solvent dissolves/releases the adhesive — gentle on both heat and force, but slow, with edge-access geometry and solvent-compatibility limits. Chosen first when both thermal and mechanical budgets are exhausted and time/chemistry can be afforded.
...
- [ ] **Step 2: §5-dominance grep + Gate-C + Debond-Matrix Ownership Test.**
  ...
  - **Method Inversion Test (user-added, sharpness check):** mask the four method NAMES — can the four paragraphs still be told apart by their arguments alone? Each must lead with a DIFFERENT first constraint (Laser→mechanical/device sensitivity · Thermal-Slide→thermal budget · Mechanical→load/force · Chemical→time & compatibility). If masked paragraphs read as interchangeable "Option A/B/C/D," the argument is taxonomy, not selection — rewrite until each method owns a distinct dominant constraint, the way Via-First/Middle/Last each own a distinct first decision.
