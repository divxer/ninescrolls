...
**Type consistency:** `ExpectedEvent { id, args }` (Task 3) consumed by `reconcileExpectedEvents`/`runExistencePage` (Task 4). `SweepMode`/`SweepPass`/state helpers (Task 2) used by `reconcileSweep` (Task 6). `persistPage(mode, pass, leaseToken, {...})` / `releaseLease(mode, pass, leaseToken, {...})` (Task 2) — the `leaseToken` 3rd arg is threaded from `acquireLease`'s return through `runPass` (Task 6). `markRollupApplied(id)` (Task 1) used by `dirtyRollupPass` (Task 5) + emit. `runExistencePage`/`runDirtyRollupPage` return `{ counters, cursor?, hasMore }` consumed identically by `runPass` (Task 6).

---

## Post-implementation review fixes (PR #226 feedback)

After the plan executed and the final holistic review passed, a further review found two items (both fixed on-branch; the task snippets above show the as-planned code — these are the deltas):
