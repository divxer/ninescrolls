...
**Status:** Design approved in principle 2026-07-09; revised after design reviews **R1–R6**. §0 precondition **RESOLVED** (single `info@` mailbox). R6: **handler-level `admin`-Cognito-group authorization** before any DynamoDB access on the touched resolvers; **first-move+marker `TransactWriteItems`** (crash-safe repair evidence from the first committed move; loser's transaction aborts → no stale marker — eliminates the 3C pre-marker residual); **release requires an unexpired lease** (expired cleanup belongs to the next acquisition); strong-read + full representative validation; marker `version` fencing for foreground-vs-drainer races; late-arrival convergence defined; literal `GmailSyncState` item + operations; fallback `sourceEntityId = ${normalizedMailbox}:${gmailMessageId}`; precise one-way dependency wording. Branch `feature/customer-360-p2-comms-sync` off `origin/main`.
...
**Crash-safe marker creation — transacted with the FIRST move (review R6/blocker-2, closes the 3C-era pre-marker window):** the first event move and the marker Put execute in **one `TransactWriteItems`**: `[ Put(movedItem, ConditionExpression: resolutionStatus='unresolved' AND orgId=<unitKey>), Put(marker) ]`. If the move condition fails (a concurrent linker won), the **whole transaction aborts — a loser never writes a marker**, so no stale-target marker can exist (the trap that killed intent-before-move in 3C). If the transaction commits, durable repair evidence exists from the very first committed move onward — a crash at ANY later point (remaining moves, contact, backfill, audit, marker bookkeeping) is healed by the drainer. Subsequent event moves proceed individually as today (idempotent). The 3C "crash-between-move-and-marker" residual is **eliminated** for every unit with ≥1 moved event.
