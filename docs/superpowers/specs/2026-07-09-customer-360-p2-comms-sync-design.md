...
**Status:** Design approved in principle 2026-07-09; revised after design reviews **R1 / R2 / R3 / R4**. §0 precondition **RESOLVED** (single `info@` mailbox). R4 replaced the SSM state design with a **fenced DynamoDB state item** (`GMAIL_SYNC#<mailbox>/STATE`, sweepState lease+CAS pattern, `BigInt` historyId comparison), made the backfill cursor **page-complete-only**, added server-side `unitKey` validation, contact-repair coverage in the 3C replay, endpoint-aware expiry classification, and literal wire contracts. Branch `feature/customer-360-p2-comms-sync` off `origin/main`.
...
6. **`linkStructuredUnit` link contract — pass the real `unitKey` (review R3/blocker-1).** Today the mutation *reconstructs* the synthetic partition as `unresolved-${sourceType}-${sourceEntityId}`; that only works when the partition key == sourceEntityId (rfq/lead/order). gmail collapses by `customerEmail`, so reconstruction would query a **nonexistent** `unresolved-gmail-<Message-ID>` partition → move nothing → no Contact. Fix: `linkStructuredUnit` (its GraphQL mutation args + handler + the Needs-Linking UI) takes the **explicit `unitKey`** the queue already returns (the partition to drain), instead of reconstructing it. Existing callers pass their `unitKey` (== the old reconstruction → behavior-identical); gmail passes `unresolved-gmail-<customerEmail>`. `sourceType` is still passed for enrichment/backfill routing. **Server-side validation (review R4 — `unitKey` is now client-supplied):** reject unless (a) `unitKey` starts with `unresolved-<sourceType>-` for the given `sourceType`; (b) for gmail, the suffix parses as a normalized email (lowercase, one `@`); (c) for non-gmail, the suffix equals the supplied `sourceEntityId`; and (d) the per-event conditional move (`resolutionStatus='unresolved' AND orgId=<unitKey>`) remains the final guard — every moved event provably belongs to that partition. A crafted `unitKey` can therefore at worst name a different *unresolved* partition, never move resolved events or touch a real org. **This touches merged 3B — enumerated so the plan owns it.**
...
- **Fenced, atomic, monotonic watermark (review R4/blocker-2 — replaces the SSM design).** The sync state lives in the DynamoDB item `GMAIL_SYNC#<mailbox>/STATE` (§2), written via the **`sweepState` pattern**: acquire a lease token at run start (conditional on no live lease), every state write is `ConditionExpression`-fenced on that token, release at end. A stale writer (old deployment function, manual invoke, expired lease-holder) fails the condition instead of rewinding state — **enforced by the store, not by ops assumptions**. `reservedConcurrentExecutions:1` remains as defense-in-depth, not the correctness mechanism. **`historyId` comparisons use decimal `BigInt`** — it's a uint64 that can exceed `Number.MAX_SAFE_INTEGER`, and string comparison is lexicographic (`"9" > "10"`); the monotonic guard compares `BigInt(new) > BigInt(stored)` inside the fenced write.
...
**Contact creation must be repair-covered (review R4):** the Contact created on link is the whole payoff for gmail (future `contact_email_exact` auto-resolution), but `manualMoveTimelineEvent` creates it *best-effort post-move*. Extend the 3C repair replay: the repair marker already carries `contactStatus`; the marker also stores the unit's `customerEmail`, and **`replayStructuredSideEffects` re-runs `upsertContact` (idempotent) whenever `contactStatus !== 'linked'` and an email is present**, updating `contactStatus` — the marker clears only when backfill + audit + **contact** are all done. Without this, a failed contact upsert is recorded but never healed, silently breaking future auto-resolution.
...
// linkStructuredUnit mutation args (3B contract change):
{ unitKey: string; sourceType: string; sourceEntityId?: string; targetOrgId: string }  // sourceEntityId optional now (gmail units have none)
```
...
- IAM (least privilege): `secretsmanager:GetSecretValue` (SA key ARN); DynamoDB `GetItem`/`PutItem`/`UpdateItem` on the intelligence table **restricted by `dynamodb:LeadingKeys = "GMAIL_SYNC#*"`** (its one state item per mailbox — §2/§4; no SSM anymore); `lambda:InvokeFunction` (crm-api) + `CRM_API_FUNCTION_NAME` env. Plus the R1/I4 credential mitigations (dedicated SA/project, `GetSecretValue` alarm, rotation).
