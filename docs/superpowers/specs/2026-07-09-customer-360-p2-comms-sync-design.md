...
- **Org merge — R10 FINAL (user-ratified 2026-07-22): merge preserves stamps; replays resolve the canonical successor.** Three candidates were rejected before this: *clearing* the stamp re-opens the R8 stale-overwrite hole; *re-stamping a fresh wall-clock ULID* loses to clock skew; the *epoch/`MERGE_OP` state machine* fixes the ordering but demands migrating every writer, a durable merge worklist, and a versioned rewrite of the production merge workflow — disproportionate to the collision. The final model changes `mergeOrganization` **as little as possible**:
  - **Merge never touches generation stamps.** It re-points `matchedOrgId`/`orgId` exactly as it does in production today; `matchedOrgLinkGeneration`/`lastLinkGeneration` survive unchanged. A leftover replay of an ALREADY-APPLIED action then supersedes naturally (`current stamp >= its generation` ⇒ `superseded` success no-op) — no new mechanism.
  - **The one real hole is the NOT-yet-applied action whose target org gets merged away.** Fix at replay time — **canonical-successor resolution** (`resolveEffectiveTarget(requestedOrgId)`): *active* target ⇒ replay normally. *Archived* target with `mergedIntoOrgId` ⇒ follow the merge chain (depth ≤ 5, visited-set cycle detection) to the active canonical successor and apply the replay THERE — the Contact still gets created, the source still gets stamped, nothing silently unfinished. *Archived* without a valid active successor (chain broken, cycle, depth exceeded) ⇒ the replay returns **`target_unavailable`** and the marker is retained **blocked/actionable** (surfaces in CRM Health) — a potentially-unfinished repair is NEVER converted into a false success.
  - **Audit truth:** the audit record preserves the ORIGINAL requested target and records the EFFECTIVE canonical target (`requestedTargetOrgId` / `effectiveTargetOrgId` in `details`) whenever they differ.
  - **The merge flow's only new obligation:** the archived source org must carry `mergedIntoOrgId` (locate in organization-api; add the attribute at archive time if the production flow doesn't already persist it). The existing archive step doubles as the assignment fence — `orgExists`/active checks already refuse new links to an archived org.
  - **Delayed source writers guarded (R10/critical):** every writer that can set `matchedOrgId` OUTSIDE link/replay/merge (rfq/lead submission, order creation/conversion — enumerate exhaustively in the plan) must condition on *stamp-absent AND org unset-or-unresolved*, so a delayed non-generational write can never overwrite a stamped admin decision (`{newOrg, staleGeneration}` pairs are unrepresentable).
