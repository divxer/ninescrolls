...
**Status: DESIGN COMPLETE — cleared for writing-plans.** Approved in principle 2026-07-09; hardened through design reviews **R1–R10** (R9: *"no Critical defects remain"*; **R10** — plan-review rounds — replaced ULID-only ordering with the epoch model. The final two architectural decisions: **ordering authority = composite `E<epoch>#<ulid>` stamps** (epoch dominates, clock-free; ULID tiebreak only for truly-concurrent same-unit actions, whose arbitrary order stays explicitly accepted; no per-unit linearization head in v1), and the **cross-writer policy** (upsertContact preserves `lastLinkGeneration`; creation-time resolution is generation-absent; link writes org+generation atomically; **merge is authoritative** via the **R10 epoch model**: all stamps are composite `E<epoch>#<ulid>`; every new authoritative action (link/relink/merge) advances the observed epoch by 1, so replays can never cross a later action regardless of clock skew — clearing OR fresh-wall-clock-ULID re-stamping were both rejected (stale-replay hole / clock-skew hole); merge itself is paginated, conditional, and retryable under a stable `MERGE_OP` id; other real-org overwrites remain forbidden)). §0 precondition **RESOLVED** (single `info@` mailbox). Plan-level items enumerated at the end of §8. Branch `feature/customer-360-p2-comms-sync` off `origin/main`.
...
**Ordering authority — decision (review R9/1, amended by R10):** ordering authority is the **composite stamp `E<epoch>#<ulid>`** (see the R10 epoch model below). Across epochs, ordering is established by conditional-write serialization in DynamoDB — **clock-free**: any action that *observed* another action's stamp mints a strictly higher epoch and deterministically wins, regardless of Lambda clock skew or same-millisecond ULID randomness. ULID order decides only WITHIN an epoch — i.e., only between *truly concurrent* actions (neither observed the other) — where the winner is **arbitrary but consistent**, explicitly accepted exactly as in R9: linkers are trusted admins, the outcome is visible in the 360/audit log, and the correction path is one click (a re-link observes the current stamp and mints a higher epoch that deterministically wins). No per-unit linearization head in v1 (deliberate YAGNI).
...
- **Org merge is authoritative — R10: epoch-advancing stamps (clock-independent ordering).** Two prior candidates both fail: *clearing* the stamp on merge leaves the record generation-absent and claimable by an older pending replay (re-opens the R8 stale-overwrite hole); *re-stamping a fresh wall-clock ULID* is insufficient because clock skew across writer Lambdas (crm-api vs organization-api) or same-millisecond random ordering can make a pre-merge generation sort AFTER the merge stamp and later undo it. The model is therefore:
  - **Stamp format:** every generation stamp (`matchedOrgLinkGeneration` AND `lastLinkGeneration`) is the composite string **`E<8-digit zero-padded epoch>#<26-char ULID>`**. No bare-ULID stamp is ever written (nothing is deployed yet — the format is native from day one); plain string comparison gives epoch dominance, ULID tiebreak within an epoch. Existing `< :gen` conditions are unchanged — stamps are opaque ordered strings to every condition.
  - **Epoch rule:** every NEW authoritative action — admin link, admin re-link, org merge — stamps each record it touches with `epoch = epochOf(observed current stamp) + 1` (absent stamp ⇒ epoch 0 observed). Epoch advancement is serialized by the conditional writes themselves, not by clocks.
  - **Replays never cross an epoch:** a replay re-asserts the EXACT stamps its action minted (persisted on the marker as per-record **`sourceGen`**/**`contactGen`** — per-record because epochs advance independently per record; `actionUlid` remains the marker/audit identity). Any authoritative action that lands after the original action makes every earlier stamp strictly smaller — a pre-merge replay is `superseded` deterministically, under any clock skew.
  - **Merge is paginated + conditional + retryable under a stable operation id:** a `MERGE_OP#<fromOrg>#<toOrg>` item holds a once-minted **`opUlid`** and a page cursor. The merge pages over the from-org's records AND contacts; each is read, then CAS-written `{ org: to, stamp: E<epoch+1>#<opUlid> }` conditioned on the observed stamp (CCFE ⇒ re-read, bounded re-attempt); a record already carrying `#<opUlid>` is skipped (idempotent); the cursor persists after each page so a crashed/retried merge resumes without re-minting or double-advancing.
  - **Later admin relink:** observes the post-merge stamp ⇒ mints epoch+2 ⇒ deterministically beats the merge, clock-free. Truly-concurrent actions (neither observed the other) tie on epoch and fall to the R9-accepted ULID tiebreak — arbitrary order is now confined to genuine concurrency instead of silently governing merge ordering.
