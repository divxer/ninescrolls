...
**Status: DESIGN COMPLETE — cleared for writing-plans.** Approved in principle 2026-07-09; hardened through design reviews **R1–R9** (R9: *"no Critical defects remain"* — the final two architectural decisions are made below: **ordering authority = ULID with explicitly-accepted arbitrary order for truly-concurrent same-unit actions** (no per-unit linearization head in v1), and the **cross-writer policy** (upsertContact preserves `lastLinkGeneration`; creation-time resolution is generation-absent; link writes org+generation atomically; **merge is authoritative** and clears the stamp; other real-org overwrites remain forbidden)). §0 precondition **RESOLVED** (single `info@` mailbox). Plan-level items enumerated at the end of §8. Branch `feature/customer-360-p2-comms-sync` off `origin/main`.
...
- **Org merge is authoritative:** `mergeOrganization` is a deliberate admin operation that re-points records; when it changes `matchedOrgId` it **atomically updates/clears `matchedOrgLinkGeneration`** in the same write (a stale stamp must never survive a merge). Enumerated as a merge-flow change in the plan.
