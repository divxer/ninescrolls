...
**Status: DESIGN COMPLETE — cleared for writing-plans.** Approved in principle 2026-07-09; hardened through design reviews **R1–R9** (R9: *"no Critical defects remain"* — the final two architectural decisions are made below: **ordering authority = ULID with explicitly-accepted arbitrary order for truly-concurrent same-unit actions** (no per-unit linearization head in v1), and the **cross-writer policy** (upsertContact preserves `lastLinkGeneration`; creation-time resolution is generation-absent; link writes org+generation atomically; **merge is authoritative** and re-stamps a **fresh generation** in the same write (R10 amendment: merely *clearing* would make the record generation-absent and claimable by an older pending replay, re-opening the stale-overwrite window R8 closed); other real-org overwrites remain forbidden)). §0 precondition **RESOLVED** (single `info@` mailbox). Plan-level items enumerated at the end of §8. Branch `feature/customer-360-p2-comms-sync` off `origin/main`.
...
**Ordering authority — decision (review R9/1):** ULID order is the system's ordering authority, with its limits **explicitly accepted**: for two *truly concurrent* admin actions on the same unit (sub-second, within Lambda clock skew), the winner is **arbitrary but consistent** — acceptable because all linkers are trusted admins, the outcome is visible in the 360/audit log, and the correction path is one click (a re-link mints a causally-later generation that deterministically wins). *Causally* later actions are correctly ordered in practice: admin actions are seconds apart while Lambda clock skew is NTP-bounded (≪1 s) — documented as an operating assumption, not silently relied upon. No per-unit linearization head in v1 (deliberate YAGNI; revisit only if multi-admin concurrent linking ever becomes real).
...
- **Org merge is authoritative:** `mergeOrganization` is a deliberate admin operation that re-points records; when it changes `matchedOrgId` it **atomically re-stamps `matchedOrgLinkGeneration` with a fresh generation ULID** in the same write (a stale stamp must never survive a merge). **R10 amendment — re-stamp, never merely clear:** clearing would leave the record generation-absent, and the replay condition (`attribute_not_exists(matchedOrgLinkGeneration) OR matchedOrgLinkGeneration < :gen`) would let an older pending repair replay claim it and re-point it back to the pre-merge org — exactly the stale-overwrite class R8 closed. A fresh merge-time ULID is greater than every pending pre-merge generation, so those replays land `superseded` and the merge stays authoritative, while genuinely new admin link actions (newer ULIDs) can still re-link. Enumerated as a merge-flow change in the plan.
