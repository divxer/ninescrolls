...
- **The Lambda's DynamoDB data-plane permission is exactly `dynamodb:Scan`, scoped by resource to the Evidence base-table ARN only.** The handler runs a `Scan` on the base table (it never `Query`s and never touches a GSI), so **neither `dynamodb:Query` nor index ARNs are granted** — either would be unused privilege, contrary to least-privilege. (The `slug` GSI is used only by the admin client's generated `listEvidenceBySlug`, which goes through AppSync's own resolver, not this Lambda.) No other DynamoDB action (`Query`/`GetItem`/`PutItem`/`UpdateItem`/`DeleteItem`/`BatchWrite`/etc.), and no access to any other table. `dynamodb:DescribeTable` is **not** granted to the Lambda in Phase 1; if a concrete implementation need arises, grant it explicitly and note it here — do not add it speculatively.
...
- A deployment/IAM check (or test) verifies both halves: (a) the Lambda's role — across **inline and attached managed policies** — grants a **non-empty** set of DynamoDB `Allow`s whose action set is **exactly `{dynamodb:Scan}`** on **exactly the deployed Evidence base-table ARN** (no `Query`, no `*`, no index ARN, no other table; assert on DynamoDB actions specifically, not "no other permissions at all"), and (b) an anonymous `apiKey` query against the **base** `Evidence` model is denied.
...
- **Public read boundary (security):**
  ...
  - **Least-privilege IAM:** assert the Lambda role's DynamoDB `Allow`s (inline + attached managed) form a non-empty set whose action set is **exactly `{dynamodb:Scan}`** on **exactly the deployed Evidence base-table ARN** (no `Query`, no writes, no `*`, no index ARNs, no other tables). The assertion targets DynamoDB actions specifically — not "the role has no other permissions" (CloudWatch Logs etc. are expected).
