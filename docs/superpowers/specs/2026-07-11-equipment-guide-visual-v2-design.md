...
- **Evidence page — LIGHT card treatment (matches the approved design; supersedes the earlier keep-dark assumption — user-corrected 2026-07-11):** light page background (no dark panel); brand-blue eyebrow; each study is a light card with a brand-blue left accent border; journal · year line in brand-blue small caps; title bold dark; platform/citations/DOI metadata line gray; disclaimer italic gray. All content strings byte-identical — `evidence.title` stays `Peer-Reviewed Validation for the Platforms We Represent` (fixture-frozen; the design mock's rephrased heading is NOT adopted). The strong-parity element contract (`data-study-index`, `.j/.y/.t/.pf/.cite/.doi`) is unchanged. Logo count consequence: the white logo variant disappears — **15 navy / 0 white** (updates the Task-5 sweep expectation).
- **Contact page — icon card grid (matches the approved design):** heading `Contact NineScrolls` (layout label, not claims copy; add to the required-strings list); the four existing data groups render as a 2×2 grid of light indigo cards with inline stroke icons (map-pin=Office Location, clock=Business Hours, mail=Contact Information, headset=Technical Support — card labels are the existing v1 headings, verbatim); CTA band above the grid; simple footer bar using existing identifiers only (logo + `NineScrolls LLC · Equipment Guide` + `ninescrolls.com · info@ninescrolls.com`). The design mock's `Powered by Precision` slogan is NOT adopted (new marketing copy — out of scope).
- **Print robustness (CSS contract):** each `PAGE_ORDER` section starts a new printed page (`break-before: page` semantics consistent with the current `.page` implementation); spec tables `break-inside: avoid` per row; chips row + CTA `break-inside: avoid`; evidence cards + contact cards `break-inside: avoid`; the CTA band prints with `-webkit-print-color-adjust: exact`.
