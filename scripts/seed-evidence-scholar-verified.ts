...
const NEW: Row[] = [
  ...
  // Conference proceedings (IEEE RAITS 2026) — excluded from launch by policy; see the classifier PROCEEDINGS set.
  ['pub-tailong-sputter-tio2-cuox-robots-raits-2026', 'TiO2-CuOx Coatings with Self-Cleaning and Antibacterial Functions and Their Potential Applications in Medical Robots', ['sputter'], '10.1109/RAITS68656.2026.11580152', 'HighThroughput100-6A', 'IEEE RAITS', 2026, 'the HighThroughput100-6A high-throughput magnetron sputtering coating machine manufactured by Tailong Electronics.'],
];
...
async function main() {
  ...
  assertUniqueSlugs(NEW.map(([slug]) => slug), 'seed-evidence-scholar-verified');
  await authenticate();
